...
CALLBACK_PREV_MONTH_CALL = 0;
CALLBACK_NEXT_MONTH_CALL = 0;
...
def handler_callback(chat_id, callback):
    ...
    """
    global CALLBACK_PREV_MONTH_CALL
    global CALLBACK_NEXT_MONTH_CALL
    if not is_inited(chat_id):
        raise Exception('inline_calendar is not inited properly')
    ...
    if callback == CALLBACK_PREVIOUS_MONTH and c_date > min_date:
        _dec_month(chat_id)
        CALLBACK_PREV_MONTH_CALL += 1
        return None
    if callback == CALLBACK_NEXT_MONTH and c_date < max_date:
        _inc_month(chat_id)
        CALLBACK_NEXT_MONTH_CALL += 1
        return None

    if callback in CALLBACK_DAYS:
        return _db_read(chat_id, _CURRENT_DATE).replace(day=int(callback.split('_')[-1]))

def get_callback_prev_month():
    global CALLBACK_PREV_MONTH_CALL
    return CALLBACK_PREV_MONTH_CALL

def get_callback_next_month():
    global CALLBACK_NEXT_MONTH_CALL
    return CALLBACK_NEXT_MONTH_CALL

def set_down_all_markers():
    global CALLBACK_NEXT_MONTH_CALL
    global CALLBACK_PREV_MONTH_CALL
    CALLBACK_NEXT_MONTH_CALL = 0
    CALLBACK_PREV_MONTH_CALL = 0
    print('Я сет даунил все маркеры')